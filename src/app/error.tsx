...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center min-h-[60vh] px-4 text-center">
      ...
      <p className="text-stone-500 text-sm mb-6 max-w-xs">{error.message || 'An unexpected error occurred.'}</p>
      <div className="flex gap-3">
        ...
        <Link
          href="/"
          className="border border-stone-200 text-stone-600 hover:border-stone-300 text-sm font-medium px-5 py-2.5 rounded-xl transition-colors"
        >
          Go home
        </Link>
      </div>
